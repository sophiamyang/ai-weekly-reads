...
- Substack is behind Cloudflare, which serves a "Just a moment..." challenge to headless browsers from datacenter IPs, GitHub Actions runners included. Browser automation therefore only works from a residential IP (a local run). The JSON API is not challenged, so CI drafts through `scripts/post_to_substack.py`, which uses the `python-substack` package and the `substack.sid` cookie.
