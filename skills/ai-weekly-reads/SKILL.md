...
- Audio download is not a workaround for blocked YouTube captions: the ios client exposes no audio-only formats, so Mistral transcription cannot rescue those items. Run locally from a residential IP, or route the transcript API through a residential proxy, when YouTube coverage matters.
